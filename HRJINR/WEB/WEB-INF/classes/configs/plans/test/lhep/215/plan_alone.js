...
function updateRooms(){
    let url = '#ServletPath#?c=plans/lhep/215/get_plans_data';
    fetch(url)
        .then((response) => {
            if (response.ok) {
                console.log('processUpdateRooms load connected');
                return response.text();
            } else {
                console.log('load #ServletPath#?c=plans/lhep/215/get_plans_data; failed');
            }
        })
        .then((data) => {
            processUpdateRooms(data);
        });
    
}
        function processUpdateRooms(data) {
            console.log('processUpdateRooms ' + data);
...
function removeSotrFromRoom(room_id,tab_n,fio,room_num,division){
       let sotr_remove = confirm("Удалить сотрудника "+ fio+ " подр. "+division+"  из комнаты "+room_num+" ?");
...
//        console.log('aaa=' + aaa);
       ...
       console.log('!!!!!!!!!! addRooms() ' + JSON.stringify(data));
...
//                 console.log('FILL ROOM '+JSON.stringify(data[i].properties))
                ...
                var room_info = getRoomInfo(room_id);
                 console.log('FILL ROOM id='+room_id+' room_num='+roomName+' room_db='+JSON.stringify(room_db))
...
//                AjaxCall('popupCont', 'c=edit/room_ppls&room_num=' +room_num+ ',&bld_name='+bld_name+'&room_id='+id, true);
                ...
                var descr = "<table  class=plan_pop_table style=\"width:600px\">" ;
                        ...
                        descr +="<input class=plan_btn type=button value='Добавить сотрудника' onclick='addUserToRoom("+room_id+","+ building_id+",\""+roomName+"\", \"#plan_building_name#\")'>";  ??rooms_addremove_ppls|rooms_edit_ppls
...
//                                descr += "<input  type=button value='Удалить'  onclick='remove_emp_from_room("+room_id+", "+room_info.PPLS[j].TABN+");' >";  ??rooms_addremove_ppls|rooms_edit_ppls
//                                descr += "</td></tr>";
...
//                                debugger;
                ...
                descr += "<tr ><td class=plan_popup_td  colspan=-1>"
                descr += "Площадь: " + parseInt(room_info.SQR/100) + ' кв.м';
                descr += "</td></tr>";
                descr +="</table";
                console.log(roomName+' Color= '+rColor+' fColor= '+fColor);
                polyline = L.polygon(latlngs, {
                    background1: "linear-gradient(to right,red 20%,        orange 20% 40%,     yellow 40% 60%,  green 60% 80%,   blue 80% )",
                        fillColor: rColor,
                        fillOpacity: 0.7,
                        color: rColor,
                        weight: 1,
                ...
                roomlbl = '<em style="color:'+fColor+'">'+roomName+'</em>';
                polyline.room_data.building_id=#plan_building_id#;
                polyline.bindTooltip(roomlbl, {
                        permanent: true,
...
function updatePopup(){
    let addsrc =  document.getElementById('rooms_ppls_table_'+this.room_data.room_id);
      AjaxCall('rooms_ppls_table_'+this.room_data.room_id, 'c=plans/lhep/215/get_edit_room_table&room_num=' +this.room_data.room_num+ '&building_id='+this.room_data.building_id+
...
function update_room_popup(room_id, building_id){
    let addsrc =  document.getElementById('rooms_ppls_table_'+room_id);
      AjaxCall('rooms_ppls_table_'+this.room_data.room_id, 'c=plans/lhep/215/get_edit_room_table&,&building_id='+building_id+
