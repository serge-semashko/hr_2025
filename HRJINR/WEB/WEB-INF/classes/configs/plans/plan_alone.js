...
function addUserToRoom(room_id,building_id,room_num,building_name){
        ...
        ShowDialog(true); 
        AjaxCall('popupCont', 'c=plans/plan_room_ppls_add&top_div_id=#top_div_id#&room_num='+room_num+'&building_name='+building_name+',&building_id='+building_id+'&room_id='+room_id, true);
    }
function rename_room(room_id,room_num){
        lit_map.eachLayer(function (layer) {
            layer.closePopup();
        });
        ShowDialog(true); 
        AjaxCall('popupCont', 'c=plans/plan_room_edit&cop=rename&room_id='+room_id+'&room_num='+room_num, true);
    }
...
function removeSotrFromRoom(room_id,tab_n,fio,room_num,division){
       ...
       if (!sotr_remove){return}
        ...
        AjaxCall('popupCont', 'c=plans/plan_room_ppls_remove&cop=remove&tab_n='+tab_n+'&room_id='+room_id, true);
...
function getRoomCoordinates(roomData) {
        ...
        for (var j = 0; j < roomData.geometry.coordinates.length; j++) {
                let a = rescaleXY( [roomData.geometry.coordinates[j][0], roomData.geometry.coordinates[j][1]]);
                roomCoordinates.push(a);
                console.log('rescaleXY '+roomData.geometry.coordinates[j][0], roomData.geometry.coordinates[j][1], rescaleXY( [roomData.geometry.coordinates[j][0], roomData.geometry.coordinates[j][1]]));
...
//        image = L.imageOverlay('images/cont.png', bounds).addTo(lit_map);
...
//                AjaxCall('popupCont', 'c=edit/room_ppls&room_num=' +room_num+ ',&bld_name='+bld_name+'&room_id='+id, true);
                ...
                var descr = "<table  class=plan_pop_table style=\"width:600px\">" ;
                        ...
                        descr += "<tr ><td width=600 class=plan_popup_td  style=\"font-family: Verdana,Arial,sans-serif !important; font-size: 1.2em !important;\" >"
...
function updatePopup(){
    let addsrc =  document.getElementById('rooms_ppls_table_'+this.room_data.room_id);
      AjaxCall('rooms_ppls_table_'+this.room_data.room_id, 'c=plans/get_room_table&room_num=' +this.room_data.room_num+ '&building_id='+this.room_data.building_id+
...
function update_room_popup(room_id, building_id){
    let addsrc =  document.getElementById('rooms_ppls_table_'+room_id);
      AjaxCall('rooms_ppls_table_'+this.room_data.room_id, 'c=plans/get_room_table&,&building_id='+building_id+
