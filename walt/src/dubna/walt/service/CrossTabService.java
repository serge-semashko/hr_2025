...
public class CrossTabService extends TableServiceSimple
{
...
//  if (makeTotals) // Вернул для блокировки тоталов 16.09.2005 - здесь колонка "всего" почему-то есть всегда - независимо от makeTotals
// убрал 10.10.2011 - в этом сервисе колонка тотал выводится всегда
  ...
  {
	/*
	  //Добавок 22.03.2012 НЕДОТЕСТИРОВАН - 
	 // 28.04.2012 убрано - не работает в Бюджет - страница для печати
		if (crossValues[i] == null) crossValues[i]="-"; 
		if (crossValues[i].indexOf("'")<0)
		  crossValues[i] = "'" + crossValues[i];
